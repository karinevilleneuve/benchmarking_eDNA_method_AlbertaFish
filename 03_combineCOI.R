...
# Script used to combine all the outputs from the methods using the COI marker gene into a single data frame 
...
path_cur = "/barque_coi/results_coicurated"
files_annotation_cur = list.files(path = path_cur, pattern=".fasta.gz\\.") # Double backslash is important to escape wildcard from period 
...
for (file in files_annotation_cur){
  ...
  annotation_clean = select(annotation, c("otu_info", "Family", "Genus", "Species", "Percentage ID"))
...
path_raw = "/barque_coi/results_coiraw"
...
for (file in files_annotation_raw){
  ...
  annotation_clean = select(annotation_rank, c("otu_info", "Family", "Genus", "Species", "Percentage ID"))
...
path_rdp = "/outputs"
files_dada2_df = as.data.frame(t(list.files(path_rdp, full.names = TRUE, pattern = "coi_dada2")))
...
list_otherspecies = c("Non-fish", "Percentage ID < 97%", "Specie bootstrap < 70", "Coregonus artedi:zenithicus")
...
df02_comnames$Sample = gsub("-PS1.*", "", df02_comnames$Sample)

# Save dataframe 
write.csv(df02_comnames,"/coi_allmethods.csv", quote = FALSE, row.names = FALSE)